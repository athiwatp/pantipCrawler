        ...
        request(options, function(err, resp, html) {
            ...
            if (title.indexOf($('.display-post-title').text()) == -1) {
                ...
                if ($('.display-post-title').text() && id) {
                    ...
                    var options = {
                        url: 'http://pantip.com/forum/topic/render_comments?tid=' + String(id).split("-")[1],
                        headers: {
                            'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.116 Safari/537.36',
                            'accept': 'application/json',
                            'X-Requested-With': 'XMLHttpRequest'
                    ...
                    request(options, function(err, resp, html) {
                        var comments = [];
                        var topicId = "topic-" + JSON.parse(resp.body).paging;
                        if (JSON.parse(resp.body).comments) {
